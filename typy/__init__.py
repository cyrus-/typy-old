...
class UsageError(TypyException):
    # TODO: rename to ExtensionBrokenError + add TypyBrokenError
    """For internal errors."""
    pass 

class TypeError(TypyException):
...
class TypeInvariantError(TypyException):
    # TODO: reconcile with ExtensionBrokenError + TypyBrokenError above
    def __init__(self, message, tree):
        Exception.__init__(self, message)
        self.tree = tree

class NotImplementedError(TypyException):
    pass
...
class KindFormationError(TypyException):
    pass
...
class TypeFormationError(TypyException):
    pass
...
class TyExpr(object): 
    ...
    def ana_kind(self, ctx, kind):
        raise NotImplementedError()

    def syn_kind(self, ctx, kind):
        raise NotImplementedError()

eq_assumptions = []
def _in_eq_assumptions(x, y):
    for (a, b) in eq_assumptions:
...
class Type(TyExpr):
    ...
    def ana_kind(self, ctx, kind):
        k = self.syn_kind(ctx)
        if k != kind:
            raise KindError("Synthesized kind " + str(k) + " != " + str(kind))
