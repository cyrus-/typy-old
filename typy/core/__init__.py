...
class UsageError(Exception):
    # TODO: rename to ExtensionBrokenError + add TypyBrokenError
    """For internal errors."""
    pass 

class TypeError(Exception):
    # TODO: rename to TyError
...
class TypeInvariantError(Exception):
    # TODO: reconcile with ExtensionBrokenError + TypyBrokenError above
    def __init__(self, message, tree):
        Exception.__init__(self, message)
        self.tree = tree

class KindFormationError(Exception):
    pass
...
class TypeKind_(Kind):
    """The kind of types."""
TypeKind = TypeKind_() # only need one
...
class TExpr(object):
    ...
    def is_atomic(self):
        raise UsageError("Missing implementation.")

class TVar(TExpr):
    """Type variable"""
    def __init__(self, ctx, id):
        if not isinstance(ctx, Context):
            raise TypeError("Must define tvar relative to a context.")
        self.ctx = ctx
        self.id = id
        if id not in ctx.tvars:
            raise TypeFormationError("Invalid type variable: " + id)
...
class TFn(TExpr):
    """Type function"""
    def __init__(self, py_body):
        if not util.is_py_fn(py_body):
            raise TypeFormationError(
                "Type function does not define a valid python function.")
        self.n_args = util.fn_argcount(py_body)
...
class TAp(TExpr):
    ...
    def __init__(self, func, args):
        if not isinstance(func, TExpr):
            raise TypeFormationError("Must apply a type expression.")
        for arg in args:
            if not isinstance(arg, TExpr):
                raise TypeFormationError("Arguments must be type expressions.")
        self.func = func
        self.args = args

    def is_atomic(self):
        return self.func.is_atomic()
